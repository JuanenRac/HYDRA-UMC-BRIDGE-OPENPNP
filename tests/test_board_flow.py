...
from hydra_umc_sdk.bridge_contract import BridgeError, BridgeJob, CellState, JobPhase, MachineState
...
class BoardFlowTests(unittest.TestCase):
    ...
    # public constructor at all. Split in two, same as
    # HYDRA-UMC-BRIDGE-UAV's own already-updated test: construction
    # rejection here, and BoardFlow.plan()'s own defensive fallback
    # covered separately with a minimal explicit double instead of
    # weakening the SDK's public contract to let the old construction
    # succeed again.
    def test_constructing_a_bridge_job_with_an_unknown_phase_is_refused_by_the_sdk_itself(self):
        with self.assertRaises(BridgeError):
            BridgeJob("pcb-job-2", "pcb-key-2", "openpnp", "SOME_FUTURE_PHASE", MachineState.IDLE, {"board_id": "pcb-42"})
    ...
    def test_a_phase_unknown_to_this_bridge_fails_safe_instead_of_crashing(self):
        ...
        # bridge's own _handoffs table hasn't been updated for yet. A real
        # BridgeJob can never carry an unmapped phase any more (see above),
        # so this uses a minimal explicit double - only the two attributes
        # plan()'s own evaluate_job() call and `_handoffs.get(...)` fallback
        # actually read - rather than a real BridgeJob the SDK would now
        # refuse to construct at all.
        unknown_phase_job = SimpleNamespace(phase="SOME_FUTURE_PHASE", machine_state=MachineState.IDLE)
