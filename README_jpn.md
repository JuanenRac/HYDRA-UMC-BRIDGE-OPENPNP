...
* 🔜 **具体的なOpenPnP拡張/API** —— インストールされたOpenPnPバージョンとマシンプロファイルをテストした後にのみ選定される。*(計画中)*
...
* **なぜ受け渡しは文字列フォーマットではなく `JobPhase` をキーとする固定辞書なのか。** `BoardFlow._handoffs` は6つのフェーズそれぞれを、文字通りの曖昧さのない説明にマッピングする。誤字や欠落したフェーズは、静かに不正なログエントリを生成する代わりに、明確に失敗する(`KeyError`)—— 追跡可能性は、各フェーズが正確に1つの名前を持つことに依存している。
...
* **なぜ具体的なOpenPnP拡張/APIが保留されているのか。** 実際にインストールされたOpenPnPバージョンとマシンプロファイルに対してテストする前に特定のプラグインインターフェースやREST面に縛られることは、このローカルコアが検証できない前提を組み込むリスクを伴う。
...
**現時点で実在するもの:** バージョン `0.0.1`。ローカルでテスト済みの追跡可能なPCB受け渡しコア(`BoardFlow`)が `HYDRA-UMC-SDK` の共有ジョブゲートの上に構築されており、決定論的な `unittest` スイートと、SDKチェックアウトを伴いCIに組み込まれた非破壊的なbuild-testスクリプトを備える。

**統合境界:** OpenPnPは常に実装キネマティクス、フィーダー制御、生の動作を保持する。このブリッジがゲート・追跡するのはあくまでその周りの*受け渡し*のみである —— ロボットによる搭載、ネイティブ実装の完了、ロボットによる取り出し。

**今後の課題:** 実際のOpenPnPマシンへの接続は主張されていない —— 具体的なOpenPnP拡張/APIは、インストールされたOpenPnPバージョンとマシンプロファイルをテストした後にのみ選定される。
