...
**現時点で実在するもの:** バージョン `0.0.6`。ローカルでテスト済みの追跡可能なPCB受け渡しコア(`BoardFlow`)が `HYDRA-UMC-SDK` の共有ジョブゲートの上に構築されており、10件の決定論的な `unittest` スイート、保存済みプロファイル検査器、可視の手動読み取り専用 OpenPnP メニューテンプレート、機械 I/O のない識別子結合シミュレーションを備える。
