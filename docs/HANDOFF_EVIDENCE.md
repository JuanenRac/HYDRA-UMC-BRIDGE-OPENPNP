...
- `identity_fingerprint`: SHA-256 fingerprint only when allowed; it does not
  expose raw board, recipe, revision or lot identifiers, nor the optional
  `slot_id`/`rack_id`/`table_id` physical-placement identifiers - all of them
  feed the fingerprint but none of them appear in the record itself.
